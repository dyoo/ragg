...
@racket[interpret-row] each @racket[row-stx].''.  Let's define
@racket[interpret-row] as well:
...
With these definitions in hand, now we can pass it syntax objects of that
form directly:

@interaction[#:eval my-eval
(interpret #'(drawing (rows (repeat 5) (chunk 3 "X") ";")))
]

or pass it the result generated by our parser:
@interaction[#:eval my-eval
(define parsed-program
  (parse (tokenize (open-input-string "3 9 X; 6 3 b 3 X 3 b; 3 9 X;"))))
